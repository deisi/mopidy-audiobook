...
class AudiobookLibraryProvide(backend.LibraryProvider):
    ...
    def __init__(self, config, backend):
        super(AudiobookLibraryProvide, self).__init__(backend)
        self._media_dirs = list(self._get_media_dirs(config))
        self._follow_symlinks = config['audiobook']['follow_symlinks']
        self._scanner = scan.Scanner(
            timeout=config['audiobook']['metadata_timeout'])
    ...
    def root_directory(self):
        if not self._media_dirs:
            ...
            uri = 'audiobook:root'
        return models.Ref.directory(name='Audiobooks', uri=uri)



    def browser(self, uri):
        logger.debug('Browsing audiobooks at: %s', uri)
        result = []
    ...
    def _get_media_dirs(self, config):
        for entry in config['audiobook']['media_dirs']:
            media_dir = {}
